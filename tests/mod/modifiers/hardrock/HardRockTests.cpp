...
{
    ResetRuntime();
    CHECK(th_mod_get_difficulty_state_v1(0) == TH_MOD_RESULT_NULL_ARGUMENT);
    CheckNeutral(ReadState());

    ThModRunConfigV1 config;
    CHECK(th_mod_get_default_config_v1(&config) == TH_MOD_RESULT_OK);
    config.enabled_mods = TH_MOD_BUILTIN_HARD_ROCK;
    ...
    CHECK(state.projectile_speed_numerator == 23);
    CHECK(state.projectile_speed_denominator == 20);
    CHECK(state.player_speed_numerator == 9);
    CHECK(state.player_speed_denominator == 10);
    CHECK(state.player_hitbox_numerator == 5);
    CHECK(state.player_hitbox_denominator == 4);
    CHECK(state.player_graze_numerator == 4);
    CHECK(state.player_graze_denominator == 5);
    CHECK(state.starting_bomb_delta == 0);
    CHECK(state.spell_time_numerator == 1);
    CHECK(state.spell_time_denominator == 1);
