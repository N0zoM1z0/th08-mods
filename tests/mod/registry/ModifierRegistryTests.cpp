...
void TestModifierRegistryV1()
{
    ...
    CHECK(th_mod::registry::BuiltinCountV1() == 9);
    ...
    const uint32_t knownMask = TH_MOD_BUILTIN_HIDDEN |
                               ...
                               TH_MOD_BUILTIN_EASY |
                               TH_MOD_BUILTIN_RELAX;
    CHECK(th_mod::registry::KnownMask(builtins, 9) == knownMask);
    CHECK(th_mod::registry::ValidateSelectionV1(
              knownMask & ~TH_MOD_BUILTIN_EASY & ~TH_MOD_BUILTIN_RELAX) ==
          TH_MOD_RESULT_OK);
    CHECK(th_mod::registry::ValidateSelectionV1(
              knownMask & ~TH_MOD_BUILTIN_HARD_ROCK &
              ~TH_MOD_BUILTIN_AUTOSHOT) == TH_MOD_RESULT_OK);
    CHECK(th_mod::registry::ValidateSelectionV1(
              TH_MOD_BUILTIN_AUTOSHOT | TH_MOD_BUILTIN_RELAX) ==
