...
bool ModifierBit(const char *token, size_t length, uint32_t &bit)
{
    ...
    if (TokenEquals(token, length, "DT") ||
        TokenEquals(token, length, "doubletime") ||
        TokenEquals(token, length, "double-time"))
    {
    ...
    if (TokenEquals(token, length, "RX") ||
        TokenEquals(token, length, "relax"))
    {
        bit = TH_MOD_BUILTIN_RELAX;
        return true;
    }
    if (TokenEquals(token, length, "BS") ||
        TokenEquals(token, length, "blindspot") ||
        TokenEquals(token, length, "blind-spot"))
    {
        bit = TH_MOD_BUILTIN_BLIND_SPOT;
        return true;
    }
    if (TokenEquals(token, length, "NB") ||
        TokenEquals(token, length, "nobomb") ||
        TokenEquals(token, length, "no-bomb"))
    {
        bit = TH_MOD_BUILTIN_NO_BOMB;
        return true;
    }
    return false;
}
...
void PrintHelp(FILE *output)
{
    fprintf(output,
            ...
            "  --mods=HD,FL,AT,MR,NF,DT,HR,EZ,RX,BS,NB  Enable a comma-separated modifier set.\n"
            ...
            "double-time, hard-rock, easy, relax, blind-spot, and no-bomb are also accepted.\n"
            "Hard Rock and Easy cannot be enabled together.\n"
            "Autoshot and Relax cannot be enabled together.\n"
